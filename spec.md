...
- **Discovery Engine & Auxiliary Search Queries:** Periodically discovers candidate items matching configured GitHub search queries (`tracked_queries`) without continuous polling overhead. Automatically injects an `updated:>` filter into searches referencing the last run timestamp minus a safety buffer (2 minutes) to account for GitHub indexing latency. Newly added queries are seeded with the query creation timestamp in SQLite metadata to avoid expensive historical backfills, and user-supplied queries with explicit `updated` qualifiers are rejected during validation. Executes lightweight, ID-only GraphQL search queries (`SearchCandidateIDs`) to find matching Issue and PullRequest node IDs. Discards previously known items via an in-memory $O(1)$ deduplication check against the SQLite item index (`GetAllItemIDs`) before issuing any hydration calls. Newly discovered candidate items are batch-hydrated (`FetchItemsByIDs`) with their full details and `viewerSubscription` state.
...
- **Subscription Mutation API:** The `UpdateSubscription` ConnectRPC handler executes the GitHub GraphQL `updateSubscription` mutation to alter subscription state on GitHub (`SUBSCRIBED`, `UNSUBSCRIBED`, `IGNORED`) and updates the local SQLite item state, recalculating computed status and returning the updated item.
...
- **Web App Dashboard (`frontend/src/`)**: Full-screen maintainer triage dashboard (Inbox, Details, Settings). Uses relative `/api/v1` routes, URL search parameters for filter state synchronization, `localStorage` for UI layout preferences, and a React `ThemeProvider` supporting GitHub-inspired Light, Dark, and System appearance modes.
  ...
  - **Settings Modal Controls (`frontend/src/Settings.tsx`)**: Provides configuration controls for multi-line `tracked_queries` (one GitHub search query per line) and `discovery_interval_min` (numeric input under Advanced settings, default 10 minutes, minimum 1 minute) with validation (including rejecting manual `updated:` filters), dirty-checking, and discard confirmation.
  ...
  - **Untracked Badge & One-Click Subscribe (`frontend/src/components/PullRequestCard.tsx`, `frontend/src/components/DetailsPane.tsx`)**: Displays an interactive "Untracked" badge on `PullRequestCard` and `DetailsPane` for items where `viewer_subscription` is `UNSUBSCRIBED`. Renders the tooltip: `"Not subscribed on GitHub. Live updates won't be received automatically unless you subscribe or are mentioned."`. Functions as a one-click button with an inline loading spinner that invokes `UpdateSubscription` to subscribe the user on GitHub and locally promote the item without navigating away.
...
* **OctoDeckService (ConnectRPC over HTTP/1.1 at `/api/v1`)**
  ...
  * `UpdateSubscription`: Update item subscription state on GitHub and in local storage.
